    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h2 id="listCar">Liste des dépense de carburants</h2>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-sm">
                        ...
                        <tbody>
                        @foreach($cars as $car)
                            <tr id="{{ str_replace(' ', '', $car->name) . $car->id . 'enabled' }}">
                                <td>{{ $car->name }}</td>
                                <td>{{ $car->fuel_type}}</td>
                                <td>{{ $car->mileage }}</td>
                                <td><button class="btn" onclick="document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'disabled' }}').className =' '; document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'enabled' }}').className =' d-none'">
                                        <i class="fas fa-pen"></i>
                                    </button>
                                </td>
                            </tr>
                            <tr id="{{  str_replace(' ', '', $car->name) . $car->id . 'disabled' }}" class="d-none">
                                <form id="{{ str_replace(' ', '', $car->name) . $car->id . 'update' }}" method="POST" action="{{ url('/car', ['id' => $car->id]) }}">
                                    @method('put')
                                    @csrf
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <button class="btn btn-outline-success" type="submit" onclick="document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'enabled' }}').className =' '; document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'disabled' }}').className =' d-none'; document.getElementById('{{ str_replace(' ', '', $car->name) . $car->id . 'update' }}').submit();">
                                            <i class="far fa-check-circle"></i>
                                        </button>
                                        <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="{{ '#' . str_replace(' ', '', $car->name) . $car->id . 'delete' }}" @if($car->fuels->count() != 0) disabled @endif>
                                            <i class="far fa-trash-alt"></i>
                                        </button>
                                        <button type="button" class="btn btn-outline-secondary" onclick="document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'enabled' }}').className =' '; document.getElementById('{{  str_replace(' ', '', $car->name) . $car->id . 'disabled' }}').className =' d-none'">
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="{{ str_replace(' ', '', $car->name) . $car->id . 'delete' }}" tabindex="-1" aria-labelledby="{{ '#' . str_replace(' ', '', $car->name) . $car->id . 'label' }}" aria-hidden="true">
